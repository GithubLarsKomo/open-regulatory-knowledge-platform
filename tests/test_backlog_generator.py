...
class TestLoadConfig(unittest.TestCase):
    ...
    def test_loads_valid_config(self):
        ...
        (self.tmpdir / "META" / "task_groups.json").write_text(
...
class TestGenerateBacklog(unittest.TestCase):
    ...
    def test_generates_tasks_from_requirements(self):
        self._write(self.tmpdir / "SPEC.md", "### REQ-CORE-0001\n")
        self._write(
            self.tmpdir / "SPEC-Claim.md", "### REQ-CLAIM-0001\n### REQ-CLAIM-0002\n"
        )
...
class TestOutputFormat(unittest.TestCase):
    ...
    def test_csv_columns(self):
        ...
        self.assertEqual(
            rows[0], ["task_id", "title", "requirement_ids", "epic", "phase"]
        )
    ...
    def test_deterministic_generation_notice(self):
        f = [
            {
                "task_id": "TASK-X-0001",
                "title": "X",
        ...
        generate_markdown_backlog(f, [], p)
        c = p.read_text(encoding="utf-8")
        self.assertIn("Generated deterministically from SPEC files.", c)
