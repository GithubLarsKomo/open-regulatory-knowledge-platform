...
class TestLifecycleTransitions:
    ...
    def test_allowed_transitions(self, repo, start, target):
        ...
        assert repo.get_by_uuid_including_deleted(obj.object_uuid).lifecycle_state in (
            target,
            "deleted",
        )
...
class TestSoftDelete:
    def test_soft_delete_from_draft(self, repo):
        ...
        assert (
            repo.get_by_uuid_including_deleted(obj.object_uuid).lifecycle_state
            == "deleted"
        )
...
class TestOptimisticLocking:
    ...
    def test_stale_lock_raises_on_transition(self, repo):
        ...
        with pytest.raises(OptimisticLockError):
            repo.transition_state(
                obj.object_uuid, "in_review", "u1", expected_lock_version=999
            )
    ...
    def test_two_session_concurrency(self):
        ...
        with pytest.raises(OptimisticLockError):
            repo_b.create_version(
                obj_b_copy.object_uuid, {"w": "v3"}, "u1", expected_lock_version=lock_b
            )
...
class TestBaseline:
    def test_create_baseline(self, repo):
        ...
        b = repo.create_baseline(
            "B1", "desc", [(o1.object_uuid, 1), (o2.object_uuid, 1)], "u1"
        )
        assert b.name == "B1"
...
class TestObjectRelation:
    def test_create_relation(self, repo):
        ...
        r = repo.create_relation(
            t.object_uuid, 1, s.object_uuid, 1, "supported_by", "u1"
        )
    ...
    def test_list_by_source(self, repo):
        ...
        repo.create_relation(t1.object_uuid, 1, s.object_uuid, 1, "supported_by", "u1")
        repo.create_relation(t2.object_uuid, 1, s.object_uuid, 1, "supported_by", "u1")
        assert len(repo.list_relations_for_source(t1.object_uuid)) == 1
        assert len(repo.list_relations_for_target(s.object_uuid)) == 2
    ...
    def test_list_by_target(self, repo):
        ...
        repo.create_relation(t.object_uuid, 1, s1.object_uuid, 1, "supported_by", "u1")
        repo.create_relation(t.object_uuid, 1, s2.object_uuid, 1, "supported_by", "u1")
        assert len(repo.list_relations_for_source(t.object_uuid)) == 2
        assert len(repo.list_relations_for_target(s1.object_uuid)) == 1
    ...
    def test_invalid_source_version_raises(self, repo):
        ...
        with pytest.raises(InvalidRelationError):
            repo.create_relation(
                t.object_uuid, 1, s.object_uuid, 999, "supported_by", "u1"
            )
    ...
    def test_invalid_target_version_raises(self, repo):
        ...
        with pytest.raises(InvalidRelationError):
            repo.create_relation(
                t.object_uuid, 999, s.object_uuid, 1, "supported_by", "u1"
            )
    ...
    def test_invalid_relation_type_raises(self, repo):
        ...
        with pytest.raises(InvalidRelationError, match="Invalid relation type"):
            repo.create_relation(
                s.object_uuid, 1, t.object_uuid, 1, "invalid_type", "u1"
            )
    ...
    def test_relation_atomic_with_invalid_version(self, repo):
        ...
        with pytest.raises(InvalidRelationError):
            repo.create_relation(_new_uuid(), 1, s.object_uuid, 1, "supported_by", "u1")
        # The first relation should still exist
        assert len(repo.list_relations_for_target(s.object_uuid)) == 1
    ...
    def test_backlog_generator_cli(self):
        ...
        try:
            ...
            for spec_file in [
                ...
                "META/REQ-META.md",
                "DATABASE/SPEC-CoreObjectStore.md",
            ...
            result = subprocess.run(
                [
                    sys.executable,
                    str(repo_root / "tools" / "backlog_generator.py"),
                    "--path",
                    str(tmpdir),
                    "--output-dir",
                    str(tmpdir / "TRACEABILITY"),
                    "--task-md",
                ],
                capture_output=True,
                text=True,
