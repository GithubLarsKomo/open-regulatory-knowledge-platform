...
class LintResult:
    ...
    definitions: Dict[str, List[Tuple[str, int]]] = field(
        default_factory=lambda: defaultdict(list)
    )
    references: Dict[str, List[Tuple[str, int]]] = field(
        default_factory=lambda: defaultdict(list)
    )
    duplicates: List[Tuple[str, List[Tuple[str, int]], int, str]] = field(
    ...
    def has_issues(self) -> bool:
        return (
            bool(self.duplicates)
            or bool(self.invalid_format)
            or bool(self.undefined_refs)
            or bool(self.errors)
        )
...
def find_markdown_files(
    root: Path, exclude_dirs: Set[str] = None, exclude_files: Set[str] = None
) -> List[Path]:
...
def generate_report(result: LintResult, repo_root: Path) -> str:
    ...
    if result.duplicates:
        ...
        lines.append("|---|---|")
        for raw_id, occurrences, ref_count, dup_type in sorted(result.duplicates):
            locs = "; ".join(f"{file}:{line}" for file, line in occurrences)
