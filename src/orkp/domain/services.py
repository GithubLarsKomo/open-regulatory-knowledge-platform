...
class DomainService:
    ...
    def update_payload(
        self, uuid_hex: str, payload: Dict[str, Any], created_by: str
        ...
        self.repo.create_version(obj.object_uuid, payload, created_by)
    ...
    def approve(
        self, uuid_hex: str, approver_user_id: str, comments: Optional[str] = None
    ) -> None:
        ...
        self.repo.transition_state(
            obj.object_uuid, "approved", approver_user_id, comments
        )
        self.repo.session.commit()
    ...
    def reject(self, uuid_hex: str, reviewer_user_id: str, comments: str) -> None:
        ...
        if obj is None:
            raise ObjectNotFoundError(f"{self.object_type} {uuid_hex} not found")
        self.repo.transition_state(
            obj.object_uuid, "rejected", reviewer_user_id, comments
        )
        self.repo.session.commit()
...
class ProductService(DomainService):
    ...
    def _check_completeness(self, uuid_hex: str) -> None:
        ...
        if obj:
            ...
            for rel_type in ("has_claim", "has_risk", "has_evidence"):
                relations[rel_type] = [
                    r for r in all_rels if r.relation_type == rel_type
                ]
        ...
        if not result["complete"]:
            ...
            if result["missing_required_fields"]:
                details.append(
                    f"Missing fields: {', '.join(result['missing_required_fields'])}"
                )
            if result["missing_relationships"]:
                details.append(
                    f"Missing relationships: {', '.join(result['missing_relationships'])}"
                )
            raise ProductCompletenessError(
                "Product does not meet minimum approval requirements. "
                + "; ".join(details)
            )
    ...
    def list_devices(self, product_uuid_hex: str) -> List[RegulatoryObject]:
        ...
        device_uuids = [
            r.source_uuid for r in relations if r.relation_type == "variant_of"
        ]
        devices = []
    ...
    def link_claim(
        self, product_uuid_hex: str, claim_uuid_hex: str, actor_user_id: str
    ) -> None:
        """Create a has_claim relation."""
        self._link_relation(
            product_uuid_hex, claim_uuid_hex, "has_claim", actor_user_id
        )

    def link_risk(
        self, product_uuid_hex: str, risk_uuid_hex: str, actor_user_id: str
    ) -> None:
        """Create a has_risk relation."""
        self._link_relation(product_uuid_hex, risk_uuid_hex, "has_risk", actor_user_id)

    def link_evidence(
        self, product_uuid_hex: str, evidence_uuid_hex: str, actor_user_id: str
    ) -> None:
        """Create a has_evidence relation."""
        self._link_relation(
            product_uuid_hex, evidence_uuid_hex, "has_evidence", actor_user_id
        )

    def _link_relation(
        self, source_hex: str, target_hex: str, rel_type: str, actor_user_id: str
    ) -> None:
    ...
    def _list_related(
        self, product_uuid_hex: str, rel_type: str
    ) -> List[Dict[str, Any]]:
        obj = self.repo.get_by_uuid_hex(product_uuid_hex)
        if obj is None:
            raise ObjectNotFoundError(f"Product {product_uuid_hex} not found")
...
class ClaimService(DomainService):
        ...
        for r in all_relations:
            if r.relation_type == "supported_by" and r.lifecycle_state == "active":
                ...
                if ev_state in ("deleted", "obsolete"):
                    ...
                    issues.append(
                        f"Evidence version {r.source_version} status is '{ev_status}', not 'approved'"
                    )
                else:
                    score += 1
    ...
    def unlink_evidence(
        ...
        target_rel = None
        for r in relations:
            if r.source_uuid == evidence_obj.object_uuid and r.relation_type in (
                "supported_by",
                "contradicted_by",
            ):
                target_rel = r
    ...
    def check_evidence_coverage(self, uuid_hex: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "reason": None
            if has_evidence
            else "No supporting evidence linked to this claim",
        }
        ...
        for r in relations:
            ...
            if ev:
                ...
                evidence_objects[_bin_to_str(r.source_uuid)] = {
                    ...
                    "quality_rating": (ev_version.payload_json or {}).get(
                        "quality_rating"
                    )
                    if ev_version
                    else None,
...
class EvidenceService(DomainService):
    ...
    def supersede_evidence(
        ...
        if replacement is None:
            raise ObjectNotFoundError(
                f"Replacement evidence {replacement_uuid_hex} not found"
            )
        ...
        blocking_claims = [
            c for c in affected_claims if c.get("relation_type") == "supported_by"
        ]
        ...
        for c in affected_claims:
            ...
            if claim:
                product_rels = self.repo.list_active_relations_for_target(
                    claim.object_uuid
                )
        ...
        return {
            ...
            "warnings": [
                f"Evidence {old_evidence_uuid_hex[:8]} superseded by {replacement_uuid_hex[:8]}"
            ],
        }
    ...
    def get_impact(self, evidence_uuid_hex: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "blocking_claim_count": len(
                [c for c in claims if c.get("relation_type") == "supported_by"]
            ),
        }
